...
if [[ "$(id -u)" -ne 0 ]]; then
  ...
  if [[ ! -w "$parent" && ! -w "$WEBROOT" ]]; then
    exec sudo --preserve-env=SITE_WEBROOT,SITE_HEALTH_URL,SITE_HEALTH_HOST,SITE_ARCHIVE,SITE_SKIP_HEALTH,SITE_DOMAIN,SITE_OVERWRITE,SITE_CADDY_DIR,SITE_ENV_FILE,CADDY_EMAIL "$0" "$@"
  fi
fi
