...
- `content/releases.md` is generated by `scripts/sync-releases.py`. `[extra.releases]` in `zola.toml` chooses the source: `mode = "github"` links the associated GitHub Releases page (`repo` / `url`); `mode = "pack"` runs `scripts/pack-source.sh` and hosts a source tarball. Do not hand-edit the page. Do not point pack-source at a forge SSH remote. Set header nav `Releases` to the GitHub URL for a direct link, or to `/releases/` for the local page.
