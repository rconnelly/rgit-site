...
The user guide (`content/docs/*.md` with `extra.generated = true`, plus `content/docs/_index.md`) is produced by `scripts/generate-docs.py` from the `doc/` tree in the rabun-git checkout. Edit that guide in rabun-git, then regenerate. Homepage copy in `content/_index.md` stays in this repo.
